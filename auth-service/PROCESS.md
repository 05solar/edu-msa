...
- 2026-09-11 — **P1-3 refresh token 회전 동시성(CLOSED)**: 회전이 조회→isUsable→revoke 로 분리돼 동일 토큰 동시 요청이 전부 성공하던 race 를 **DB 조건부 UPDATE(consumeIfUsable — `set revoked=true where id=? and revoked=false`)** 로 원자화 — 반환 1건=승자, 0건=패자(JVM 락 불사용, replica 수 무관). 재현 실측: 수정 전 staging(6 replica)에서 동일 쿠키 10요청 → **10개 전부 200·active 토큰 10개**, 단위 테스트도 2/2·8/10 성공. 폐기 토큰 재제출 정책 정련(V2: `revoked_at` 신설): reuse-grace(기본 30s, `edu.auth.reuse-grace-seconds`) **안**의 재제출=동시 경쟁 패배(정상, 401만 — 승자 세션 유지) / grace **밖** 또는 시각 불명=탈취 의심(기존대로 계정 전 세션 폐기) / **만료만으로는 전 세션을 끊지 않음**(방치 탭의 만료 쿠키는 공격 신호 아님 — 종전엔 이 경우도 전 계정 세션이 끊겼다). 트랜잭션은 짧은 상태 전환뿐(bcrypt 없음·조건부 UPDATE 행 잠금만). 검증: gradle 전체 통과(RefreshRotationConcurrencyTest 7건 — 동시 2/10·grace 재사용·백데이트 탈취·만료·logout·위조), staging 실측 — 수정 후 동일 쿠키 10동시 → **정확히 1×200/9×401·active 1**, 순차 재사용 401+승자 회전 지속, 서로 다른 토큰 20 병렬 452ms 전건 200·deadlock 0, Flyway V2 는 실데이터 있는 CNPG(기존 DB 경로)에 적용 확인.
- 2026-09-10 — Redis 클라이언트 타임아웃 단축(readiness 3차): spring.data.redis timeout 2s→250ms + connect-timeout 500ms. staging 장애 드릴 실측 — 로그인 1회가 rate-limit 카운터 연산을 여러 번 수행해 Redis 장애 시 +10s 지연되던 것을 0.40s 로 개선(폴백·차단 의미 불변, fail-open 아님 재확인). 검증: gradle build + staging 장애 재드릴. 함께 auth HPA(2–6)+PDB 가 autoscale.yaml 에 신설됨(로그인 용량 실측 replica당 ~16/s 근거 — deploy/PROCESS.md).
- 2026-09-10 — 로그인 트랜잭션 분리(production readiness review): `login()` 의 `@Transactional` 제거 — bcrypt(~50ms)가 트랜잭션 안에서 DB 커넥션을 점유해 400 로그인/s 에서 풀 고갈되던 실측 원인(11단계) 제거. 계정 조회는 리포지토리 짧은 읽기 트랜잭션, 토큰 저장은 save() 짧은 쓰기 트랜잭션으로 분리(이 경로는 엔티티 무변경이라 묶음 트랜잭션 불요). `refresh()` 는 회전·탈취 감지 원자성 필요로 유지. 검증: gradle build 15/15 + staging(kind 멀티노드) 무중단 롤아웃 후 ID/PW·데모·refresh 회전·오답 401 전 경로 실검증, 30/s burst 통과분 p95 59ms·Hikari active/pending 0.
- 2026-09-09 — rate-limit Redis 분산 카운터(8단계): RedisAttemptStore(INCR+EXPIRE/SET EX/TTL/DEL, 키 접두어 edu:rl:) — 다중 replica 가 계정/IP 카운터·차단 상태를 공유해 동일 정책 적용. FailoverAttemptStore 로 Redis 장애 시 인메모리 폴백(fail-open 아님, edu.auth.ratelimit.failover 카운터 + 1분 간격 경고). 선택은 edu.auth.ratelimit.store(memory 기본, K8s 는 redis 주입). actuator prometheus 노출 추가(Redis 헬스는 자동 포함). 검증: FailoverAttemptStoreTest 2건 + RedisAttemptStoreTest 3건 + gradle build 15/15.
