...
5. **인가는 JWT `role` 클레임으로만.** 인증은 auth-service 책임이고 backend는 자원
   서버다. 접근 규칙은 `security.SecurityConfig` 한 곳에만 두고, `user` 도메인 데이터로
   인가 판단을 하지 않는다. 엔드포인트 추가 시 필요한 접근 등급(공개/로그인/CODER/ADMIN)을
   명시한다.
6. **이모지 금지** (루트 규칙과 동일).
