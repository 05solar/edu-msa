...
- 프로그램 하나를 **내부 Gitea** 레포지토리 하나에 담습니다. 외부 저장소(GitHub 등)는
  사용하지 않습니다 — 코드가 기관 밖으로 나가지 않아야 하며, 플랫폼도 내부 Gitea
  주소만 받도록 강제합니다(`EDU_DEPLOY_GITEA_ONLY`).
- Gitea 계정은 플랫폼 **마이페이지 → Gitea 계정** 패널에서 직접 발급합니다
  (영문 아이디 · 비밀번호 8자 이상). 코드는 Gitea 웹 화면의 **파일 업로드**로
  올리거나 `git push` 로 올립니다.
- 그 레포 주소(예: `https://gitea.edu.internal/내계정/my-tool`)를 플랫폼의
  "프로그램 등록" 화면에 붙여넣으면 됩니다.
- **비공개 레포**(신규 레포 기본값)는 배포 봇(`edu-deploy-bot`)을 협업자(read)로
  초대하면 배포됩니다. push 하면 자동으로 재배포됩니다(main 브랜치).
...
- [ ] 내부 Gitea 에 레포가 있고 주소를 안다 (비공개면 `edu-deploy-bot` 초대 완료)
...
- 헬스 경로 없음 → `/healthz` 추가.
- 비공개 레포에 봇 미초대 → `edu-deploy-bot` 을 협업자(read)로 초대하거나 공개로 전환.
- 외부 저장소(GitHub 등) 주소 등록 → 내부 Gitea 주소만 등록할 수 있습니다.

## 6. 등록하는 법

1. 위 규칙에 맞춰 내부 Gitea 레포에 올립니다(웹 파일 업로드 또는 `git push`).
2. 플랫폼에 로그인합니다. **프로그램 등록에는 "바이브 코더(CODER)" 권한이 필요**합니다.
   가입은 항상 일반 사용자(USER)로 되며, 마이페이지에서 코더 권한을 신청하고 운영 관리자
   승인을 받으면 등록할 수 있습니다.
3. 플랫폼 → "프로그램 등록" → 내부 Gitea 레포 주소 입력 → 브랜치 선택.
