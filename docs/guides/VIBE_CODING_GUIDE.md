...
1. 레포지토리(내부 Gitea 권장, GitHub 병용) 하나 = 서비스 하나
...
- 프로그램 하나를 레포지토리 하나에 담습니다. **내부 Gitea(권장)** 또는 외부
  GitHub(병용)를 씁니다 — 내부망에서는 코드가 외부로 나갈 수 없으므로 Gitea 를 권장합니다.
- 그 레포 주소(예: `https://gitea.edu.internal/내계정/my-tool` 또는
  `https://github.com/yourname/my-tool`)를 플랫폼의
  "프로그램 등록" 화면에 붙여넣으면 됩니다.
- 내부 Gitea 의 **비공개 레포**도 배포 봇(`edu-deploy-bot`)을 협업자(read)로
...
- [ ] 레포(내부 Gitea 권장 · GitHub 병용)가 있고 주소를 안다
