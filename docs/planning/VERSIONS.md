...
| `v0.7.0` | 2026-09-04 | Gitea 내부 저장소 — 1~6단계 완료·kind 통합 검증 (§3) |
| `v0.8.0` | 2026-09-09 | 확장성 개조 — SCALABILITY_REVIEW 권장 조치 13단계 + staging 실검증 (7단계) |
...
2026-08-24 ~ 09-09, 시간 순서대로 8단계로 진행했다.
...
### 6단계 — Gitea 내부 저장소 (09-01~09-04, `v0.7.0`)

- §3 계획 1~6단계 전체 구현·kind 통합 검증 후 main 병합·태깅.
- 내부 Gitea 레포 주소만으로 등록→검증→토큰 clone→Kaniko 빌드→배포→공개 전 구간 동작,
  push 시 webhook(HMAC 서명 검증) 자동 재배포까지 확인.

### 7단계 — 확장성 개조 (09-09, `v0.8.0`)

[SCALABILITY_REVIEW.md](../operations/SCALABILITY_REVIEW.md)(수십만 사용자 적합성 검토)의
§5 권장 조치 13단계를 순서대로 전부 구현·검증했다. 상세 이력은 각 폴더 PROCESS.md 참고.
...
| 우선순위 | 항목 | 내용 | 비고 |
|---|---|---|---|
| ~~A~~ | ~~**Gitea 도입**~~ | §3 전체 — 1~6단계 구현·kind 통합 검증·main 병합 완료 | `v0.7.0` 태깅됨 |
| ~~A~~ | ~~**시크릿 관리**~~ | Sealed Secrets 표준 + placeholder 반입 차단(fail-closed) 완료 — 주기 회전 절차만 잔여 | 7단계(9단계 조치) |
| B | 배포 직렬화 | 동일 슬러그 동시 배포 직렬화(두 워커가 같은 Kaniko Job 경합) — RUNNING 고아 작업 회수는 7단계 requeueStale 로 해소 | 6단계 통합 검증 발견 |
| B | 로그인 용량 | auth-service HPA 신설 + 로그인 트랜잭션에서 bcrypt 분리(커넥션 점유 해소) | 7단계 부하 실측 권장 |
