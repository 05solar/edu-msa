...
각 서비스의 루트 경로(`/`)는 API 안내가 아니라 **실제 업무용 웹 화면**을 제공합니다.
통계 카드 · 검색/상태 필터 · 목록 테이블 · 등록 폼 · 상태별 처리 버튼으로 구성되며,
프론트의 "웹에서 바로 사용" 링크로 접속하면 바로 사용할 수 있습니다. (API는 동일 서비스의 `/api/*` 로 계속 제공)
